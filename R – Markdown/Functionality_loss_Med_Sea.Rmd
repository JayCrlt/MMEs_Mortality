...
One way to visualize which traits are the most impacted is to use a Sankey diagram. See below (interactive)
...
Sankey_dataset_traits <- Sankey_dataset %>%
  group_by(Category) %>% 
  summarise(value = sum(Occurence)/3, source = Trait) %>% 
...
Sankey_dataset <- Sankey_dataset %>% mutate(value = value * 1548)

nodes = data.frame(name= unique(c(Sankey_dataset$source, Sankey_dataset$target)))

Sankey_dataset$IDsource=match(Sankey_dataset$source, nodes$name)-1 
Sankey_dataset$IDtarget=match(Sankey_dataset$target, nodes$name)-1

Sankey_dataset <- Sankey_dataset %>% data.frame()
...
-   [**slow growth organisms**]{.underline}\
